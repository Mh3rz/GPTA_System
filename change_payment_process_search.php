<?php
    ...
    if ($_SERVER['REQUEST_METHOD'] === 'POST') {
        ...
        if ($storedHashedPassword && password_verify($enteredPassword, $storedHashedPassword)) {
            ...
            $stat = 'change';
            header("Location: search.php?search=".$searchQuery. "&page=" . $currentPage ."&stat=".$stat."&message=" . $row['lastname'] . ", " . $row['firstname'] . ". Status changed back to <b>Not Paid</b>!");
            exit;
        } else {
            // Password is incorrect
            $stat = 'error';
            header("Location: search.php?search=".$searchQuery. "&page=" . $currentPage ."&stat=".$stat."&message= Incorrect Password");
        }
    }
